...
var getDayTimestamp = function(offset) {
  return parseInt(
    this.moment().startOf('day').subtract(offset, 'day').format('x'), 10
  );
};

var metrics = function(moment) {
  this.moment = moment;
};
...
metrics.prototype.sumDays = function(timeseries, dayStart, dayEnd) {
  var sum = 0;
  var i = 0;
  var day = this.moment(timeseries[0].date);
  ...
  while (day && day.isBefore(dayEnd)) {
    ...
    i++;
    day = timeseries[i] ? this.moment(timeseries[i].date) : null;
  }
...
metrics.prototype.getDay = function(offset, description, timeseries) {
  ...
  if (dayIndex != -1) {
    var day = timeseries[dayIndex];
    var dayBefore = timeseries[dayIndex - 1];
    return {
      description: description,
      value: day.value,
...
metrics.prototype.weekAgo = function(timeseries, offset) {
  offset = offset || 0;
  var week1Start = this.moment().startOf('week').subtract(offset, 'week');
